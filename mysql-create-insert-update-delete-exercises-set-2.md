# MySQL CREATE, INSERT, UPDATE, and DELETE Practice Pack — Set 2

## Purpose

This pack contains 10 new standalone-table exercises. It does not reuse the tables from the earlier student, product, customer, book, patient, bank-account, vehicle, hotel-room, movie, or support-ticket exercises.
...
SQL solutions are intentionally omitted.
...
- Use MySQL 8.0.16 or later so that `CHECK` constraints are enforced.
- Use a separate practice database.
- Do not use real personal, financial, insurance, or business data.
- This pack intentionally contains no transaction exercises or transaction statements.
- Before every `UPDATE` or `DELETE`, execute a `SELECT` using the same `WHERE` condition.
- Treat each deletion as permanent and verify the target carefully.
- Never omit a `WHERE` clause from an `UPDATE` or `DELETE` unless an exercise explicitly requires every row to be affected.
...
All policy information below is fictional.
...
> Real insurance systems normally retain or archive policy history. Physical deletion here is strictly for database practice.
...
- [ ] No transaction statements were used.
- [ ] Final table contents were verified.

# Suggested Assessment Rubric

| Area | Marks |
|---|---:|
| Table creation and data types | 15 |
| Primary-key constraints | 5 |
| Unique constraints | 10 |
| `NOT NULL` constraints | 10 |
| Default values | 10 |
| Named `CHECK` constraints | 15 |
| Valid and invalid inserts | 15 |
| Correct updates | 10 |
| Safe deletes with precise conditions | 10 |
| **Total** | **100** |
